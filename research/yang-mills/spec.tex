...
{\small\input{generated/requirements.tex}}
...
\input{generated/interpretations.tex}
...
{\small\input{generated/assumptions.tex}}
...
{\small\input{generated/transitions.tex}}
...
\paragraph{A direct attempt.} Research Report 2 (\texttt{reports/report-02.pdf}) passes nine candidate mechanisms through the sanity tests of Section~\ref{sec:verification} and works through the multiscale Bakry--Émery criterion of Bauerschmidt, Bodineau and Dagallier (\idref{A-041}), the strongest current tool for non-convex regimes. For lattice Yang--Mills it meets a gauge dilemma (\idref{A-042}), a conversion problem (\idref{A-044}) and, decisively, the need for large-scale convexification of the renormalized potential (\idref{A-043}), which is the mass gap in multiscale form. No solution results.

\paragraph{The length of the bridge.} Research Report 3 (\texttt{reports/report-03.pdf}) measures with a Monte Carlo renormalization group that only 4--6 factor-2 blocking steps lead from the scaling regime ($\beta=2.2$--$2.6$) into the proved region $\beta<1/12$ (\idref{A-046}, numerical). All other RG steps lie at weak coupling. The non-perturbative core is therefore a bounded, $a$-independent number of steps at intermediate coupling (\idref{A-047}); it remains open.
...
\bibitem[BBD24]{BBD24} R.~Bauerschmidt, T.~Bodineau, B.~Dagallier, Stochastic dynamics and the Polchinski equation: an introduction, \emph{Probab. Surveys} 21 (2024). doi:10.1214/24-PS27
