...
def at21_plaquette(beta):
    """Linear interpolation of the AT21 plaquette inside their beta range, else NaN."""
    bs = [row[0] for row in AT21]
    if not bs[0] <= beta <= bs[-1]:
        return float('nan')
    return float(np.interp(beta, bs, [row[2] for row in AT21]))
...
def main():
    ...
    with open(os.path.join(OUT, 'glueball.dat'), 'w') as f:
        ...
        for r in runs:
            ...
            # Same rule as the table: a value whose error exceeds it is not plotted.
            m1, dm1 = (m[1], dm[1]) if not math.isnan(m[1]) and dm[1] < abs(m[1]) else (float('nan'), float('nan'))
            f.write(f"{r['beta']} {fmt(m[0])} {fmt(dm[0])} {fmt(m1)} {fmt(dm1)} {fmt(ratio)} {fmt(dratio)}\n")
    ...
    def pm(x, dx, digits):
        ...
        if math.isnan(x) or math.isnan(dx) or abs(dx) >= abs(x):
            return '---'
        if dx > 0:  # show at least one significant digit of the error
            digits = max(digits, -math.floor(math.log10(abs(dx))))
        return f'${x:.{digits}f}({round(abs(dx) * 10 ** digits):d})$'
    with open(os.path.join(OUT, 'table.tex'), 'w') as f:
        ...
        for r in runs:
            ...
            m, dm = r['glueball_meff'], r['glueball_meff_err']
            series = uu + 4 * uu ** 5
            ref = at21_plaquette(b)
            ref_txt = '---' if math.isnan(ref) else f'${ref:.5f}$'
            f.write(f"{b:.1f} & {pm(p, dp, 5)} & ${series:.5f}$ & ${100 * (p - series) / p:+.2f}$ & {ref_txt} & {pm(c[0], dc[0], 3)} & {pm(c[1], dc[1], 3)} & {pm(m[1], dm[1], 2)}\\\\\n")
    ...
    devs = [(r['beta'], abs(r['plaquette'][0] - (u(r['beta']) + 4 * u(r['beta']) ** 5)) / r['plaquette'][0]) for r in runs]
    last_ok = max((b for b, d in devs if d < 0.002), default=float('nan'))
    first_bad = min((b for b, d in devs if d > 0.01), default=float('nan'))
    print(f"strong-coupling series within 0.2% up to beta={last_ok}; off by >1% from beta={first_bad}")

    # Agreement with the reference plaquette and precision of our glueball estimates.
    cmp = [(abs(r['plaquette'][0] - at21_plaquette(r['beta'])), r['plaquette'][1]) for r in runs if not math.isnan(at21_plaquette(r['beta']))]
    plaq_diff = max(d for d, _ in cmp)
    plaq_sigma = max(d / e for d, e in cmp)
    rel = [r['glueball_meff_err'][1] / abs(r['glueball_meff'][1]) for r in runs
           if not math.isnan(r['glueball_meff'][1]) and r['glueball_meff_err'][1] < abs(r['glueball_meff'][1])]
    print(f"plaquette vs AT21 (interpolated): max |diff| {plaq_diff:.1e} = {plaq_sigma:.1f} sigma; glueball relative errors {min(rel):.0%}-{max(rel):.0%}")

    with open(os.path.join(OUT, 'constants.tex'), 'w') as f:
        f.write('% Generated by lab/analyze.py. Do not edit by hand.\n')
        f.write(f"\\newcommand{{\\SeriesOkUpTo}}{{{last_ok}}}\n")
        f.write(f"\\newcommand{{\\SeriesFailsFrom}}{{{first_bad}}}\n")
        f.write(f"\\newcommand{{\\PlaqMaxDiff}}{{{plaq_diff * 1e4:.1f}}}\n")
        f.write(f"\\newcommand{{\\PlaqMaxSigma}}{{{plaq_sigma:.1f}}}\n")
        f.write(f"\\newcommand{{\\GlueRelErrMin}}{{{100 * min(rel):.0f}}}\n")
        f.write(f"\\newcommand{{\\GlueRelErrMax}}{{{100 * max(rel):.0f}}}\n")
