...
\textbf{Validation.} A built-in self-test checks the quaternion algebra against $2\times2$ matrices, the heat-bath distribution against the exact first moment, gauge invariance and exact action conservation under over-relaxation. The full simulation reproduces the strong-coupling series $\langle\tfrac12\Tr U_p\rangle=u+4u^5+\dots$, $u=I_2(\beta)/I_1(\beta)$, within errors at $\beta=0.5$ and $\beta=1.0$ (Table~\ref{tab:lab}), and approaches the weak-coupling form $1-3/(4\beta)$ at large $\beta$ (Figure~\ref{fig:plaq}). \input{data/repro.tex}
...
\toprule
$\beta$ & $\langle\frac12\Tr U_p\rangle$ & $u+4u^5$ & $\chi(2)$ & $\chi(3)$ & $am_{\mathrm{eff}}(1)$ & $m_{\mathrm{eff}}/\sqrt{\chi(3)}$\\
\midrule
\input{data/table.tex}
\end{tabular}
\caption{Own simulation, $SU(2)$, Wilson action, $\LatticeL^4$. A dash marks a value whose statistical error exceeds the value itself. At strong coupling the glueball correlator and the larger Wilson loops fall below the noise, as expected for heavy states and a large string tension. $\chi(3)$ still contains perimeter and Coulomb contributions; it is used only as a rough physical scale.}\label{tab:lab}
\end{table}
...
\addplot[only marks, mark=*, mark size=1.6pt, blue!70!black, error bars/.cd, y dir=both, y explicit] table[x=beta, y=m1, y error=dm1] {data/glueball.dat};
...
\addplot[only marks, mark=*, mark size=1.6pt, blue!70!black, error bars/.cd, y dir=both, y explicit] table[x=beta, y=ratio, y error=dratio] {data/glueball.dat};
...
\legend{$m_{\mathrm{eff}}/\sqrt{\chi(3)}$ (this work), $m_G/\sqrt\sigma$~\cite{AT21}}
\end{axis}
\end{tikzpicture}
\caption{Left: the mass gap in lattice units falls towards zero as $\beta$ grows (it must, since $a\to0$). Right: in physical units it stays roughly constant. This is the numerical picture of \idref{A-031} and of rule \idref{Q-3}; it is evidence, not proof.}\label{fig:gap}
\end{figure}
