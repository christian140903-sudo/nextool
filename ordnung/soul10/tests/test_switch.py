"""Schalter: trivial und Formatzwang erkannt, keine Falsch-Positiven auf oder/besser/live/user, Log ohne Prompttext, Datensatz ≥ 40 Prompts ≥ 90 %."""
import json
...
# --- Datensatz: Prompt, Soll-Stufe (ohne Sonde) ------------------------------------------------
DATENSATZ = [
    # direkt: trivial (kurz, ein Satz, kein Signal)
    ("Was ist die Hauptstadt von Frankreich?", "direkt"),
    ("What is the capital of France?", "direkt"),
    ("Danke!", "direkt"),
    ("ok", "direkt"),
    ("Wie viele Tage hat ein Schaltjahr?", "direkt"),
    ("How many legs does a spider have?", "direkt"),
    ("Uebersetze 'Guten Morgen' ins Englische.", "direkt"),
    ("Translate 'thank you' into German.", "direkt"),
    ("Nenne das chemische Symbol fuer Gold.", "direkt"),
    ("Was ergibt 15 % von 200?", "direkt"),
    ("Is 97 a prime number?", "direkt"),
    ("Wandle 5 km in Meilen um.", "direkt"),
    ("Welcher Wochentag war der 1. Januar 2000?", "direkt"),
    ("Give me a synonym for 'happy'.", "direkt"),
    ("Kaffee oder Tee?", "direkt"),
    ("Welches Wort ist besser: schnell oder rasch?", "direkt"),
    ("I live in Vienna, what time zone is that?", "direkt"),
    ("Wie spaet ist es in Tokio, wenn es in Berlin 12 Uhr ist?", "direkt"),
    # direkt: Formatzwang
    ("Berechne 2+2, nur die Zahl.", "direkt"),
    ("Rechne 17*23. Antworte nur mit der Zahl.", "direkt"),
    ("Return only the number.", "direkt"),
    ("Gib die Liste als JSON aus.", "direkt"),
    ("Output the result as JSON only.", "direkt"),
    ("Sortiere diese Woerter alphabetisch und gib NUR die sortierte Liste aus: Birne, Apfel, Kirsche.", "direkt"),
    ("Wie viele Buchstaben 'e' enthaelt 'Erdbeere'? Gib NUR die Zahl aus.", "direkt"),
    ("Liste: 4, 9, 12, 7, 20, 3, 8. Wie viele Zahlen sind gerade? Antworte NUR mit der Anzahl als Zahl.", "direkt"),
    ("Convert this table to CSV. CSV only, no explanation.", "direkt"),
    ("Welches Wort steht an dritter Stelle in 'der schnelle braune Fuchs'? Gib nur das Wort aus, kein weiterer Text.", "direkt"),
    ...
    ("Ein Zug faehrt um 8:15 Uhr mit 90 km/h los, ein zweiter um 9:00 Uhr mit 120 km/h auf derselben Strecke. "
     ...
     "10 Minuten Pause macht.", "aufwand"),
    ("Lisa hat dreimal so viele Aepfel wie Tom. Zusammen haben sie 48. Tom gibt Lisa 6 Aepfel. Wie viele hat Lisa jetzt?", "aufwand"),
    ("A tank fills at 3 L/min and drains at 1.2 L/min. It starts with 20 L and has a capacity of 100 L. "
     "After how many minutes is it full?", "aufwand"),
    ("Wenn ein Auto 3 Stunden mit 80 km/h faehrt und dann 2 Stunden mit 100 km/h, wie weit kommt es insgesamt?", "aufwand"),
    ("Ich habe drei Angebote fuer eine Wohnung: 900 Euro kalt mit 15 Minuten Pendelzeit, 750 Euro mit 40 Minuten, "
     "1100 Euro mit 5 Minuten. Ich verdiene 2800 Euro netto. Welche sollte ich nehmen?", "aufwand"),
    ("Prove that the sum of the first n odd numbers is n squared.", "aufwand"),
    ...
    ("Sollen wir die API nach Prod migrieren oder erst das Datenmodell umbauen?", "aufwand"),
    ("Should we deploy the new schema to production on Friday or wait until Monday?", "aufwand"),
    ("Entwirf eine Architektur fuer ein Ereignis-Bus-System, das 10.000 Nachrichten pro Sekunde verarbeitet.", "aufwand"),
    ("Wie sollte ich mein Team ueber die Kuendigung eines Kollegen informieren?", "aufwand"),
    ("Refactor this function so it handles both empty and null input; keep the public interface unchanged.", "aufwand"),
    ("Vergleiche PostgreSQL und SQLite fuer ein lokales Werkzeug mit einem Nutzer: Was empfiehlst du und warum?", "aufwand"),
    ("We need to decide whether to hire a second backend engineer or invest in automation first. What would you recommend?", "aufwand"),
    ("Plane die Migration von drei Datenbanken auf eine neue Version, mit Rollback-Schritten und Zeitplan.", "aufwand"),
    ("Write a design document for the new customer onboarding flow, covering data model, API contract and rollout.", "aufwand"),
    ("Schreibe eine Absage an einen Bewerber, freundlich, aber eindeutig, und erklaere kurz die Gruende.", "aufwand"),
    ("Draft a message to the team announcing that the release is delayed by two weeks.", "aufwand"),
    ("Is it ethical to delete the user's data without telling them? Explain your reasoning.", "aufwand"),
    ("Soll ich die Stelle kuendigen und mich selbststaendig machen?", "aufwand"),
]


def test_datensatz_groesse_und_sprachen():
    assert len(DATENSATZ) >= 40
    assert sum(1 for p, _ in DATENSATZ if p.split()[0].lower() in
               ("what", "how", "is", "translate", "give", "i", "return", "output", "a", "prove",
                "should", "refactor", "we", "write", "draft", "convert")) >= 12
    assert {s for _, s in DATENSATZ} == {"direkt", "aufwand"}


def test_trefferquote_datensatz_mindestens_90_prozent():
    fehler = []
    for prompt, soll in DATENSATZ:
...
def test_zwei_saetze_sind_nicht_trivial():
    ...
    assert switch.decide("Berechne den Umfang eines Kreises mit Radius 3. Runde auf zwei Stellen.")["stage"] == "aufwand"
...
def test_signale_ohne_falsch_positive():
    ...
    for name in ("tradeoff", "recommendation", "production", "affects_others"):
        ...
        assert not rx.search("oder") and not rx.search("besser") and not rx.search("live") and not rx.search("user")
