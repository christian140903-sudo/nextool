...
| `consolidate.py` | Takt A (Stop und Sitzungsende, mechanisch: Inbox → Episoden), Takt B (Stop-Hook, höchstens alle 6 h: Dubletten, Widerspruch in Herkunftsordnung — stärkster aktiver Eintrag bleibt, gleich stark → beide `disputed`, ein Kandidat gewinnt nie —, Retention `exp(−Δt/strength)`, Archiv < 0,1, Kandidaten nach einem Tag ohne Widerspruch aktiv). Ohne Modellaufruf lauffähig. | ungemessen (Rang 2 der offenen Fragen) | D031–D032; jeder Schreibvorgang trägt `derived_from`. |
