...
const behaviorlockSource = readFileSync(join(root, 'behaviorlock/index.html'), 'utf8');
...
if (!/Behaviorlock 0\.1\.0/.test(behaviorlockSource) || !/>73 \/ 73</.test(behaviorlockSource) || !/>99\.06%</.test(behaviorlockSource)) {
  errors.push('Behaviorlock product page release evidence is stale');
}
if (/npmjs\.com\/package\/behaviorlock/.test(behaviorlockSource)) {
  errors.push('Behaviorlock product page claims an npm surface before registry publication');
}
